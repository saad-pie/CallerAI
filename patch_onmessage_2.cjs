const fs = require('fs');
const code = fs.readFileSync('src/components/CallView.tsx', 'utf8');

const replacement = `           onmessage: (msg: LiveServerMessage) => {
             ...
             if (parts) {
               ...
               if (textAcc) {
                 ...
                 setTranscript(prev => [...prev, {
                   speaker: aiContact.name || aiContact.number,
                   text: textAcc,
                   timestamp: Date.now()
                 }]);
               }
...
const regex2 = /onmessage: \(msg: LiveServerMessage\) => \{[\s\S]*?if \(textAcc\) \{[\s\S]*?\}\]\);\s*\}\s*\}\s*\}/g;
let i = 0;
let newCode = code.replace(regex2, (match) => {
  if (i === 1) return replacement;
  i++;
  return match;
});

fs.writeFileSync('src/components/CallView.tsx', newCode);
console.log("Patched second occurrence.");
